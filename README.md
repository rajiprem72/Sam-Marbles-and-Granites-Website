# Sam Marbles & Granites Website

A modern, responsive and SEO-friendly business website developed for **Sam Marbles & Granites**, Kolathur, Chennai.

This website has been created as a token of gratitude to **Dr. C. M. Victor Emmanuel**, whose timely help and support during a critical medical emergency saved my life. This project is my humble way of expressing my heartfelt thanks.

---

## 🌐 Website Features

* Mobile First Responsive Design
* Modern Premium UI
* HTML5, CSS3 & JavaScript
* Product Gallery
* Customer Testimonials
* Google Maps Integration
* Click-to-Call
* One-Tap WhatsApp Chat
* One-Tap Email with Pre-filled Subject & Message
* Smooth Scrolling Animations
* SEO Optimized
* GitHub Pages Ready
* Fast Loading
* Easy to Maintain

---

## 📄 Website Pages

* Home
* About Us
* Product Gallery
* Testimonials
* Contact Us

---

## 🏢 Business Information

**Sam Marbles & Granites**

No.19, Ayyappan Nagar
200 Feet Road
Opposite DR Furnitures
Kolathur
Chennai – 600099
Tamil Nadu, India

**Proprietor**

Dr. C. M. Victor Emmanuel

📞 Phone: +91 98848 52288

📧 Email: [drvictoremmanuel@yahoo.com](mailto:drvictoremmanuel@yahoo.com)

---

## 🏛 About Sam Marbles & Granites

Serving customers for over **20 years**, Sam Marbles & Granites offers a wide collection of premium marbles, granite and designer granites sourced from renowned suppliers across Andhra Pradesh and Rajasthan.

Located on the main road in Kolathur, Chennai, the showroom is easily accessible and is situated opposite DR Furnitures and diagonally opposite DR Hospital.

Customers can expect quality products, competitive pricing and personalized service.

---

## 🍽 Dziner's Kitchen

Located within the same premises, **Dziner's Kitchen** specializes in beautiful modular kitchen solutions.

Their services include:

* Kitchen Planning
* Modular Kitchen Design
* 3D Kitchen Concepts
* Custom Cabinet Solutions
* Complete Kitchen Execution

Together, Sam Marbles & Granites and Dziner's Kitchen help customers create elegant and functional living spaces under one roof.

---

## 🛠 Technologies Used

* HTML5
* CSS3
* JavaScript
* Google Fonts
* Font Awesome
* Google Maps
* GitHub Pages

---

## 🚀 Deployment

This project is designed for deployment using **GitHub Pages**.

Simply upload the repository and enable GitHub Pages from the repository settings.

---

## 📱 Mobile Friendly

The website is optimized for smartphones, tablets and desktops, ensuring an excellent browsing experience across all screen sizes.

---

## 📈 SEO Features

* Semantic HTML
* Meta Description
* Open Graph Tags
* Responsive Design
* Fast Loading
* Clean URL Structure
* Optimized Images
* Search Engine Friendly

---

## 📄 License

This project has been developed exclusively for **Sam Marbles & Granites**.

---

## ❤️ A Note of Gratitude

This website is more than a business website—it is a heartfelt expression of gratitude.

Dr. C. M. Victor Emmanuel extended timely help during one of the most difficult moments of my life. His quick action ensured I received immediate medical attention, and I remain deeply thankful.

May this website contribute, in some small way, to the continued success and growth of Sam Marbles & Granites.

---

**Developed with gratitude by**

**Premanandhan Narayanan**
